...
const App: React.FC = () => {
  ...
  const handleNewYear = () => {
    ...
    const updatedTeams = gameState.teams.map(t => {
        if (t.jokerActive === 'BANK') {
            return { ...t, capital: t.capital - 10000 }; // 10% of 100k
        }
        return t;
    });
  ...
  if (clientTeamId) {
      ...
      // Allow Team to Select Industry via props callback if strictly needed, 
      // but simpler to handle inside ClientView for this demo OR update state here:
      // We will enhance TeamClientView to allow "messaging" or assume the GM sets it manually in the update panel
      // For a better experience, we can let TeamClientView assume it's "set" locally until refreshed.
